...
class MyCursor {
	...
	public void setType(int type, int indexOfButton // only used if (type ==
													// TYPE_INTERACTING_WITH_WIDGET)
	) {
		this.type = type;
...
class PaletteButton {
	...
	public void draw(int palette_x, int palette_y, // upper left corner of the
		...
		if (label.equals("-") || label.equals("+") || label.equals("Thin") || label.equals("Medium")
			...
			gw.drawIcons(palette_x, palette_y, true, label);
		}
	}
}
...
class Palette {
	...
	public Palette() {
		...
		b = new PaletteButton(3 * W, H, "Hor. Flip", "Flip the selection horizontally (around a vertical axis).",
				false);
		horizFlip_buttonIndex = buttons.size();
		buttons.add(b);

		b = new PaletteButton(4 * W, H, "Ver. Flip", "Flip the selection vertically (around a horizontal axis).",
				false);
...
class UserContext {
	...
	public void movePalette(float delta_x, float delta_y // displacement, in
															// pixels
	) {
	...
	// returns true if any cursors are currently being handled by this user
	// context
	...
	// returns the distance between the given point and the center of the
	// palette of this user context
	public float distanceToPalette(float x, float y // pixel coordinates
	) {
	...
	public boolean processMultitouchInputEvent(int id, float x, // in pixels
			float y, // in pixels
			int type, GraphicsWrapper gw, boolean doOtherUserContextsHaveCursors) {
		// Find the cursor that corresponds to the event id, if such a cursor
		// already exists.
			...
			if (cursor.type == MyCursor.TYPE_NOTHING) {
				...
				if (type == MultitouchFramework.TOUCH_EVENT_UP) {
					...
					if (cursorContainer.getNumCursorsOfGivenType(MyCursor.TYPE_CAMERA_PAN_ZOOM) == 2) {
						...
						gw.pan(cursor.getCurrentPosition().x() - cursor.getPreviousPosition().x(),
				...
				if (type == MultitouchFramework.TOUCH_EVENT_UP) {
					...
					if (cursorContainer.getNumCursorsOfGivenType(MyCursor.TYPE_DIRECT_MANIPULATION) == 2) {
						...
						Point2D cursor0_currentPosition_worldSpace = gw
								.convertPixelsToWorldSpaceUnits(cursor0.getCurrentPosition());
						Point2D cursor1_currentPosition_worldSpace = gw
								.convertPixelsToWorldSpaceUnits(cursor1.getCurrentPosition());
						Point2D cursor0_previousPosition_worldSpace = gw
								.convertPixelsToWorldSpaceUnits(cursor0.getPreviousPosition());
						Point2D cursor1_previousPosition_worldSpace = gw
								.convertPixelsToWorldSpaceUnits(cursor1.getPreviousPosition());
						...
						for (Stroke s : selectedStrokes) {
							Point2DUtil.transformPointsBasedOnDisplacementOfTwoPoints(s.getPoints(),
									...
									cursor0_currentPosition_worldSpace, cursor1_currentPosition_worldSpace);
							s.markBoundingRectangleDirty();
						}
						drawing.markBoundingRectangleDirty();
					} else if (cursorContainer.getNumCursorsOfGivenType(MyCursor.TYPE_DIRECT_MANIPULATION) == 1) {
						// convert cursor positions to world space
						Point2D cursor_currentPosition_worldSpace = gw
								.convertPixelsToWorldSpaceUnits(cursor.getCurrentPosition());
						Point2D cursor_previousPosition_worldSpace = gw
								.convertPixelsToWorldSpaceUnits(cursor.getPreviousPosition());
...
public class SimpleWhiteboard implements Runnable, ActionListener {
	...
	public SimpleWhiteboard(MultitouchFramework mf, GraphicsWrapper gw) {
		...
		if (Constant.NUM_USERS == 1) {
			userContexts[0].setPositionOfCenterOfPalette(Constant.INITIAL_WINDOW_WIDTH / 2,
					Constant.INITIAL_WINDOW_HEIGHT / 2);
		} else {
	...
	// Returns the index of the user context that is most appropriate for
	// handling this event.
	...
	public synchronized void processMultitouchInputEvent(int id, float x, float y, int type) {
		...
		boolean redrawRequested = userContexts[indexOfUserContext].processMultitouchInputEvent(id, x, y, type, gw,
				doOtherUserContextsHaveCursors);
		if (redrawRequested)
			multitouchFramework.requestRedraw();
	}
}
